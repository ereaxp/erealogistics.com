...
<main inert={mobileOpen || undefined}>
  ...
  <section id="about" class="px-container py-section">
    <div class="mx-auto max-w-7xl">
      ...
      <div class="grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
        <div use:stagger={{ selector: '.about-paragraph', stagger: 0.1 }} class="surface-card p-7 md:p-9">
        ...
        <div class="grid gap-4">
          ...
          <div use:reveal={{ delay: 0.08 }} class="surface-card-soft p-6" data-reveal>
            <p class="text-sm uppercase tracking-[0.12em] text-text-tertiary">{t.hero.note}</p>
          </div>
        </div>
      </div>
    </div>
  ...
  <section id="methodology" class="px-container py-section-sm">
    <div class="mx-auto max-w-7xl rounded-[2rem] bg-[linear-gradient(180deg,var(--color-accent-deep),#0A3A27)] px-6 py-10 text-white shadow-[0_36px_90px_rgba(0,80,47,0.24)] md:px-10 md:py-12">
      ...
      <div class="grid gap-8 lg:grid-cols-[0.95fr_1.05fr]">
        ...
        <div use:stagger={{ selector: '.value-item', stagger: 0.09 }} class="grid gap-4 sm:grid-cols-2">
          {#each t.value.items as item}
            <article class="value-item rounded-[1.35rem] border border-white/14 bg-white/8 p-5">
        ...
        {#each t.transformation.items as item}
          <article class="transformation-card surface-card flex h-full flex-col p-6">
            <div class="mb-6 flex items-center justify-between">
              <span class="inline-flex min-h-11 min-w-11 items-center justify-center rounded-full border border-border-subtle bg-[color:var(--color-bg-secondary)] font-serif text-lg text-[color:var(--color-accent-deep)]">{item.number}</span>
            </div>
            ...
            <p class="text-sm text-[color:var(--color-accent-deep)]">{item.impact}</p>
          </article>
        {/each}
      </div>
    </div>
  ...
  <section id="impact" class="px-container py-section-sm">
    <div class="mx-auto max-w-7xl">
      <SectionHeader label={t.impact.label} number="04" />
      <div class="mb-8 grid gap-5 lg:grid-cols-[0.9fr_1.1fr] lg:items-end">
        <h2 use:reveal class="text-h2 max-w-[11ch] font-serif tracking-snug" data-reveal>{t.impact.title}</h2>
        <p use:reveal={{ delay: 0.08 }} class="text-body-lg text-text-secondary" data-reveal>{t.impact.description}</p>
      </div>

      <div use:stagger={{ selector: '.impact-card', stagger: 0.06 }} class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {#each t.impact.items as item}
          <article class="impact-card surface-card-soft p-6">
            <div class="mb-4 font-serif text-counter tracking-tight text-[color:var(--color-accent-deep)]">{item.value}</div>
            <div class="mb-4 h-px w-12 bg-[color:var(--color-accent)]"></div>
      ...
      <div class="grid gap-8 lg:grid-cols-[0.82fr_1.18fr]">
        ...
        <div use:stagger={{ selector: '.question-item', stagger: 0.05 }} class="grid gap-3 md:grid-cols-2">
          {#each t.questions.items as item}
            <div class="question-item flex gap-3 rounded-[1.1rem] border border-border-subtle bg-white/70 px-4 py-4">
              <div class="mt-2 h-2.5 w-2.5 flex-none rounded-full bg-[color:var(--color-accent)]"></div>
  ...
  <section id="cases" class="px-container py-section-sm">
    <div class="mx-auto max-w-7xl">
      <SectionHeader label={t.cases.label} number="06" />
      <div class="mb-8 max-w-[52rem]">
        <h2 use:reveal class="text-h2 mb-4 max-w-[10ch] font-serif tracking-snug" data-reveal>{t.cases.title}</h2>
        <p use:reveal={{ delay: 0.08 }} class="text-body-lg text-text-secondary" data-reveal>{t.cases.description}</p>
      </div>
      ...
      <div use:stagger={{ selector: '.case-card', stagger: 0.08 }} class="grid gap-4 md:grid-cols-2">
        {#each t.cases.items as item}
          <article class="case-card surface-card p-6 md:p-7">
            <div class="metric-chip mb-5 inline-flex text-sm font-semibold text-[color:var(--color-accent-deep)]">{item.highlight}</div>
            <h3 class="text-h3 mb-3 font-serif">{item.title}</h3>
            <p class="text-body text-text-secondary">{item.text}</p>
          </article>
        {/each}
  ...
  <section id="team" class="px-container py-section">
    <div class="mx-auto max-w-7xl">
      <SectionHeader label={t.team.label} number="07" />
      <div class="mb-8 max-w-[52rem]">
        <h2 use:reveal class="text-h2 mb-4 max-w-[12ch] font-serif tracking-snug" data-reveal>{t.team.title}</h2>
      </div>

      <div class="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
        <div use:stagger={{ selector: '.team-paragraph', stagger: 0.08 }} class="surface-card p-7 md:p-9">
        ...
        <div use:reveal class="surface-card ambient-panel p-7 md:p-9" data-reveal>
          ...
          <div class="grid gap-3">
            {#each t.team.items as item}
              <div class="rounded-[1.15rem] border border-border-subtle bg-white/72 p-4">
                <div class="mb-2 font-medium text-[color:var(--color-accent-deep)]">{item.title}</div>
  ...
  <section id="contact" class="px-container py-section-lg">
    <div class="mx-auto max-w-7xl">
      <SectionHeader label={t.contact.label} number="08" />
      <div class="mb-8 max-w-[52rem]">
      ...
      <div class="grid gap-6 lg:grid-cols-[1fr_0.9fr]">
        <form onsubmit={handleDemoSubmit} class="surface-card p-7 md:p-9">
          <div class="grid gap-5 md:grid-cols-2">
            <label class="block text-sm font-medium text-text-primary">
              ...
              <input class="w-full rounded-[1rem] border border-border-subtle bg-[color:var(--color-bg-card)] px-4 py-3 outline-none" name="name" placeholder={t.contact.form.namePlaceholder} required />
            </label>

            <label class="block text-sm font-medium text-text-primary">
              <span class="mb-2 block">{t.contact.form.companyLabel}</span>
              <input class="w-full rounded-[1rem] border border-border-subtle bg-[color:var(--color-bg-card)] px-4 py-3 outline-none" name="company" placeholder={t.contact.form.companyPlaceholder} required />
            </label>

            <label class="block text-sm font-medium text-text-primary">
              <span class="mb-2 block">{t.contact.form.emailLabel}</span>
              <input class="w-full rounded-[1rem] border border-border-subtle bg-[color:var(--color-bg-card)] px-4 py-3 outline-none" name="email" type="email" placeholder={t.contact.form.emailPlaceholder} required />
            </label>

            <label class="block text-sm font-medium text-text-primary">
              <span class="mb-2 block">{t.contact.form.topicLabel}</span>
              <input class="w-full rounded-[1rem] border border-border-subtle bg-[color:var(--color-bg-card)] px-4 py-3 outline-none" name="topic" placeholder={t.contact.form.topicPlaceholder} required />
            </label>
          </div>

          <label class="mt-5 block text-sm font-medium text-text-primary">
            <span class="mb-2 block">{t.contact.form.messageLabel}</span>
            <textarea class="min-h-[132px] w-full rounded-[1rem] border border-border-subtle bg-[color:var(--color-bg-card)] px-4 py-3 outline-none" name="message" placeholder={t.contact.form.messagePlaceholder}></textarea>
          </label>
        ...
        <div class="grid gap-4">
          <div use:reveal class="surface-card ambient-panel p-7 md:p-9" data-reveal>
            ...
            <div class="mt-6 grid gap-3">
              {#each t.contact.expectations.bullets as bullet}
                <div class="flex items-center justify-between rounded-[1rem] border border-border-subtle bg-white/72 px-4 py-3">
                  <span class="text-sm font-medium text-[color:var(--color-accent-deep)]">{bullet}</span>
          ...
          <div use:reveal={{ delay: 0.08 }} class="surface-card-soft p-6" data-reveal>
            ...
            <a class="text-body font-medium text-[color:var(--color-accent-deep)]" href={`mailto:${t.contact.email}?subject=${encodeURIComponent(t.contact.emailSubject)}`}>
              {t.contact.email}
            </a>
          </div>
        </div>
